 ...
 session_start(); 
 //ログアウト
 echo "<form action=mission_3-7.php method=post>";
...
if($_FILES["upfile"]["name"] !== ""){
    ...
    $sql->bindParam(':data', $img, PDO::PARAM_STR);
    $sql->execute();
...
while ($row = $result -> fetch(PDO::FETCH_ASSOC)){
		...
		if($row["kakutyosi"] == "mp4"){
            echo ("<video src='import_media.php?target=$target' width='426px' height='240px' controls></video>");
			echo "<br />\n";
        }
